...
export default class Monitor extends EventEmitter {
  ...
  initMonitor = async () => {
    try {
      ...
      const response = await axios.get(
        `${this.site}${process.env.SHOPIFY_END_POINT}`,
        {
          headers: safeHeaders,
  ...
  monitorLoop = async () => {
    try {
      ...
      this.previousProducts.forEach((product) => {
        matchedProductIndex = this.currentProducts.findIndex(
          (_product) => _product.id === product.id
        );
        matchedProduct = this.currentProducts[matchedProductIndex];
        if (
          matchedProduct &&
          product.updated_at !== matchedProduct.updated_at
        ) {
          this.checkRestocks(
            this.currentProducts[matchedProductIndex],
            product
          );
        }
      });
      ...
      this.previousProducts.forEach((product) => {
        matchedProductIndex = _currentProducts.findIndex(
          (_product) => _product.id === product.id
        );
  ...
  checkRestocks = async (product, oldProduct) => {
    ...
    product.variants.forEach((variant) => {
      if (
        variant.available &&
        !oldProduct.variants.find((_variant) => _variant.id === variant.id)
          .available
      ) {
