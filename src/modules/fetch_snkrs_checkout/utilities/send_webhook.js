 ...
 */
export default async function sendWebhook(checkOutUrl, productTitle, size) {
  // The Discord webhook
  const webhookUrl = process.env.SNKRS_CHECKOUT_DISCORD;

  try {
    // Create embed structure object message to send to Discord
    const embed = {
        title: `${productTitle} - Early Checkout Link`,
        color: 5763719,
        author: {
          name: '7tinh Hub',
        ...
        fields: [
          ...
          {
            name: ``,
            value: `[Early Checkout Link](${checkOutUrl})`,
            inline: true,
          }
        ],
        footer: {
